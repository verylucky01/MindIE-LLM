...
HealthChecker::HealthChecker() : mRunning(false) {
    ...
    ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
              "HealthChecker: Healthchecker instance created.");
}

void HealthChecker::GetChipPerCard() {
    std::string cmd =
        "npu-smi info -t usages -i 0 | awk '/Chip Count/ {print $NF}'";
    try {
        std::string output = ExecuteCommand(cmd);
        mChipPerCard = std::stoi(output);
        if (mChipPerCard <= 0) {
            ULOG_WARN(SUBMODLE_NAME_HEALTHCHECKER,
                      GenerateHealthCheckerErrCode(
                          WARNING, SUBMODLE_FEATURE_SECURE, CHECK_ERROR),
                      "HealthChecker: Invalid Chip Count value from npu-smi: "
                          << output << ". Defaulting to 1.");
            mChipPerCard = 1;
        } else {
            ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
                      "HealthChecker: Detected Chip Count: " << mChipPerCard);
        }
    } catch (const std::exception &e) {
        ULOG_WARN(SUBMODLE_NAME_HEALTHCHECKER,
                  GenerateHealthCheckerErrCode(WARNING, SUBMODLE_FEATURE_SECURE,
                                               CHECK_ERROR),
...
void HealthChecker::PrintNpuDeviceIds() {
    ...
    ss << "}";
    ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
              "HealthChecker: Device Card IDs (card:chip): " << ss.str());
}
...
HealthChecker::~HealthChecker() {
    ...
    // 停止虚推任务
    {
        if (mSimulateRunner != nullptr && mSimulateTaskStarted.load()) {
            ...
            ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
                      "HealthChecker: Stopped simulate task.");
        }
        mSimulateRunner.reset();
        mSimulateExecutor.reset();
    }
    ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
              "HealthChecker: Healthchecker instance destroyed.");
}
...
bool HealthChecker::Start() {
    if (!mRunning.load()) {
        ...
        ULOG_DEBUG(SUBMODLE_NAME_HEALTHCHECKER,
                   "HealthChecker: Health check thread started.");
        return true;
    } else {
        ULOG_WARN(SUBMODLE_NAME_HEALTHCHECKER,
                  GenerateHealthCheckerErrCode(WARNING, SUBMODLE_FEATURE_SECURE,
...
void HealthChecker::Stop() {
    if (mRunning.load()) {
        ...
        ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
                  "HealthChecker: Health check thread stopped.");
    } else {
        ULOG_WARN(
...
ServiceStatus HealthChecker::GetServiceStatus() {
    return mServiceStatus.load();
}

bool HealthChecker::CheckErrorListEmpty() {
    return ErrorQueue::GetInstance().Size() == 0;
}

void HealthChecker::GetStatusAndErrorList(ServiceStatus &status,
    ...
    ULOG_DEBUG(SUBMODLE_NAME_HEALTHCHECKER,
               "HealthChecker: GetStatusAndErrorList called. Status: "
...
bool HealthChecker::WaitForLlmEngineReady() {
    ...
    while (mRunning.load()) {
        ...
        if (currentStatus != SERVICE_INIT) {
            ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
                      "HealthChecker: Status already changed to "
        ...
        ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
                  "HealthChecker: Init finished, update status to normal");
        UpdateStatus(SERVICE_NORMAL);
        return true;
    }
    return false;
...
void HealthChecker::PerformPeriodicHealthCheck() {
    ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
              "HealthChecker: Starting health check loop with interval: "
    ...
    while (mRunning.load()) {
        ...
        // 检测状态变化，处理虚推任务的暂停/恢复
        bool wasPauseOrReady = (previousStatus == SERVICE_PAUSE ||
                                previousStatus == SERVICE_READY);
        bool isPauseOrReady =
            (currentStatus == SERVICE_PAUSE || currentStatus == SERVICE_READY);
    ...
    if (!GetInferInstance()->IsLlmEngineReady()) {
        ...
        if (llmNotReadyCount >= MAX_ENGINE_NOT_READY_COUNT) {
            ULOG_WARN(SUBMODLE_NAME_HEALTHCHECKER,
                      GenerateHealthCheckerErrCode(
                          WARNING, SUBMODLE_FEATURE_SECURE, CHECK_ERROR),
                      "HealthChecker: LLM engine not ready for "
                          << llmNotReadyCount
                          << " times, will report health status.");
        }

        mServiceStatus.store(SERVICE_NORMAL);
        std::string errCode = GenerateHealthCheckerErrCode(
            INFO, SUBMODLE_FEATURE_SECURE, SIMULATE_NORMAL);
        ErrorQueue::GetInstance().EnqueueErrorMessage(
            errCode, SUBMODLE_NAME_HEALTHCHECKER);
...
void HealthChecker::SetSendingMessageStatus(
    bool sendingMessageStatus) noexcept {
...
void HealthChecker::HandleHealthStatus() {
    ...
    if (status == SERVICE_ABNORMAL) {
        if (!mFirstSimulateAbnormalSuppressed) {
            ...
            errCode = GenerateHealthCheckerErrCode(
                INFO, SUBMODLE_FEATURE_SECURE, SIMULATE_NORMAL);
        } else if (isSending) {
            status = SERVICE_NORMAL;
            errCode = GenerateHealthCheckerErrCode(
            ...
            errCode = GenerateHealthCheckerErrCode(
                ERROR, SUBMODLE_FEATURE_SECURE, STATUS_WARNING);
        }
    } else if (status == SERVICE_NORMAL || status == SERVICE_BUSY) {
        mFirstSimulateAbnormalSuppressed = false;
        errCode = GenerateHealthCheckerErrCode(INFO, SUBMODLE_FEATURE_SECURE,
                                               SIMULATE_NORMAL);
    }
    ...
    ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
              "HealthChecker: The simulate infer health check result is "
...
void HealthChecker::CheckServiceStatus() {
    ...
    if (!StartSimulateTask()) {
        ULOG_ERROR(SUBMODLE_NAME_HEALTHCHECKER,
                   GenerateHealthCheckerErrCode(ERROR, SUBMODLE_FEATURE_SECURE,
                                                CHECK_ERROR),
    ...
    ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
              "HealthChecker: Exiting health check loop.");
}
...
std::shared_ptr<ISimulateExecutor> HealthChecker::CreateSimulateExecutor() {
    ...
    InferReqType reqType = (serverConfig.inferMode == INFER_MODE_DMI)
                               ? InferReqType::REQ_PREFILL
                               : InferReqType::REQ_STAND_INFER;
    std::string mode =
        (serverConfig.inferMode == INFER_MODE_DMI) ? "DMI" : "Standard";

    ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
              "HealthChecker: Creating simulate executor for "
...
bool HealthChecker::CreateAndInitSimulateRunner() {
    ...
    if (mSimulateExecutor == nullptr) {
        ...
        if (mSimulateExecutor == nullptr) {
            ULOG_ERROR(SUBMODLE_NAME_HEALTHCHECKER,
                       GenerateHealthCheckerErrCode(
                           ERROR, SUBMODLE_FEATURE_SECURE, CHECK_ERROR),
    ...
    SimulateTaskRunner::RunMode runMode =
        (mIsCentralizedNode && !mIsCentralizedMaster)
            ? SimulateTaskRunner::RunMode::NPU_ONLY
            : SimulateTaskRunner::RunMode::SIMULATE_AND_NPU;
    if (!mSimulateRunner->Init(mSimulateExecutor, mNpuDeviceCardIds,
                               mNPUThreshold, runMode, mChipPerCard)) {
        ULOG_ERROR(SUBMODLE_NAME_HEALTHCHECKER,
                   GenerateHealthCheckerErrCode(ERROR, SUBMODLE_FEATURE_SECURE,
                                                CHECK_ERROR),
...
bool HealthChecker::InitNpuDeviceCardIds() {
    ...
    const auto loadNpuIdsFromBackend = [this, &npuDeviceIds,
                                        chipsPerCard]() -> bool {
        ...
        for (const auto &id : npuDeviceIds[0]) {
            ...
            // 时每进程只采样本进程 chip。
            mNpuDeviceCardIds.emplace_back(lid / chipsPerCard,
                                           lid % chipsPerCard);
        }
...
ServiceStatus HealthChecker::CheckSimulateTask() {
    ULOG_DEBUG(SUBMODLE_NAME_HEALTHCHECKER,
               "HealthChecker: Starting simulate inference check.");
        ...
        switch (healthStatus.lastStatus) {
            ...
            default:
                ULOG_WARN(
                    SUBMODLE_NAME_HEALTHCHECKER,
                    GenerateHealthCheckerErrCode(
                        WARNING, SUBMODLE_FEATURE_SECURE, CHECK_WARNING),
...
bool HealthChecker::IsValidStatusTransition(const ServiceStatus &from,
                                            const ServiceStatus &to) {
    if (statusTransferMap.find(from) == statusTransferMap.end() ||
        std::find(statusTransferMap[from].begin(),
                  statusTransferMap[from].end(),
                  to) == statusTransferMap[from].end()) {
        return false;
    }
    return true;
}
...
void HealthChecker::UpdateStatus(const ServiceStatus &status) {
    ...
    if (mServiceStatus.load() == status) {
        ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
                  "HealthChecker: Status unchanged: "
                      << StatusToString(mServiceStatus));
        return;
    }
    if (!IsValidStatusTransition(mServiceStatus.load(), status)) {
        ULOG_ERROR(SUBMODLE_NAME_HEALTHCHECKER,
                   GenerateHealthCheckerErrCode(ERROR, SUBMODLE_FEATURE_SECURE,
                                                CHECK_ERROR),
                   "HealthChecker: Invalid status transition from "
                       << StatusToString(mServiceStatus.load()) << " to "
                       << StatusToString(status));
        return;
    }
    ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
              "HealthChecker: Status changed from "
                  << StatusToString(mServiceStatus.load()) << " to "
                  << StatusToString(status));
    mServiceStatus.store(status);
}

void HealthChecker::EnqueueErrorMessage(
    const std::string &errCode, const std::string &createdBy,
    const std::chrono::time_point<std::chrono::system_clock> &timestamp) {
    ErrorQueue::GetInstance().EnqueueErrorMessage(errCode, createdBy,
                                                  timestamp);
    ULOG_INFO(SUBMODLE_NAME_HEALTHCHECKER,
              "HealthChecker: New error added. Error code: "
...
void HealthChecker::UpdateNpuDeviceIds(const std::set<int> &npuDeviceIds) {
    const int chipsPerCard = (mChipPerCard > 0) ? mChipPerCard : 1;
    {
        ...
        for (const auto &id : npuDeviceIds) {
            mNpuDeviceCardIds.emplace_back(id / chipsPerCard,
                                           id % chipsPerCard);
        }
    }
    PrintNpuDeviceIds();
...
SimulateResult HealthChecker::RunHttpTimedHealthCheck(uint32_t waitTime) {
    ...
    ULOG_DEBUG(SUBMODLE_NAME_HEALTHCHECKER,
               "HealthChecker: RunHttpTimedHealthCheck. reqType="
                   << static_cast<int>(reqType) << ", waitTime=" << waitTime
                   << "s");
