...
void DmiRole::TaskThread()
{
    ...
    while (!taskTerminate_.load()) {
        ...
        if (taskRunning_.load()) {
            ...
            // In a real scenario, you might need more sophisticated cancellation
...
void DmiRole::QueryLinkStatus()
{
    ...
    if (linkingLinkIP_.empty() && runningLinkIP_.empty() && waittingLinkIP_.empty()) {
    ...
    const auto& waittingLinks = response.waitting_link_info();

    ULOG_INFO(SUBMODLE_NAME_ENDPOINT, "Processing link status: failed_links=" << failedLinks.size() <<
              ", success_links=" << successLinks.size() <<
              ", running_links=" << runningLinks.size() <<
              ", waitting_links=" << waittingLinks.size());

    runningLinkIP_.clear();
    waittingLinkIP_.clear();
    for (const auto& runningLink : runningLinks) {
        runningLinkIP_.emplace_back(runningLink);
    }
    for (const auto& waittingLink : waittingLinks) {
        waittingLinkIP_.emplace_back(waittingLink);
    }
    ...
    // Set role to READY if linking table is empty and success table is not empty
...
void DmiRole::ProcessFailedLinks(const T& failedLinks)
{
    for (const auto& failedInfo : failedLinks) {
        try {
            ...
            // Always update the status for failed instances, regardless of linking state
            // This handles cases where backend reports failures for instances not currently tracked
            ...
            if (linkingLinkIP_.find(instanceId) != linkingLinkIP_.end()) {
                ...
                std::string failMsg = "Link failed for instance id: " + std::to_string(instanceId) +
                                      " with error code: " +
            ...
            ULOG_ERROR(SUBMODLE_NAME_ENDPOINT, GenerateEndpointErrCode(ERROR, SUBMODLE_FEATURE_SPLITWISE,
                CHECK_ERROR), "Invalid cluster_id in failed link info: " << failedInfo.cluster_id());
        }
    }
}
...
{
    if (linkingLinkIP_.empty() && !successLinkIP_.empty() && runningLinkIP_.empty() && waittingLinkIP_.empty()) {
...
)
{
    ...
    for (auto it = this->successLinkIP_.cbegin(); it != this->successLinkIP_.cend(); ++it) {
        ...
        if (currentLinkIpInfo.find(key) == currentLinkIpInfo.cend()) {
            ULOG_INFO(SUBMODLE_NAME_ENDPOINT, "Unlink successLinkIP_ key: " << key << " value: "
                << this->successLinkIP_[key].size());
            globalIpInfo.unlinkIpInfo[key] = this->successLinkIP_[key];
            this->successLinkIP_.erase(key);
            if (superPodId != "") {
    ...
    // links being established also need to check if they need to be disconnected
    // this->linkingLinkIP_ - currentLinkIpInfo = unlink
...
{
    // Clean up remoteNodeLinkStatus_ for instances that are no longer in success or linking
    ...
    std::map<uint64_t, std::vector<std::string>>& currentLinkHostIpInfo)
{
    ...
    // The link item mathematical operation is as below:
    // currentLinkHostIpInfo - this->successHostIP_ - this->linkingHostIP_ = link
    ...
    GlobalIpInfo &globalIpInfo)
{
    ...
    // Each request can proceed independently but must coordinate on shared state
    {
        std::lock_guard<std::mutex> lock(mtx_);
...
const std::map<uint64_t, std::vector<std::string>> &DmiRole::GetLinkingHostIp()
{
    ...
    const std::map<uint64_t, std::pair<std::string, bool>>& dpInstanceStatusMap)
{
    ...
    for (const auto& entry : dpInstanceStatusMap) {
        ...
        if (!dpInstanceStatus) {
            instanceStatus[instanceId] = {"dp instance id : " + std::to_string(dpInstanceId)
