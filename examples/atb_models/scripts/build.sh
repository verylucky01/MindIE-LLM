...
USE_VERBOSE=OFF
IS_RELEASE=0
BUILD_OPTION_LIST="third_party unittest pythontest fuzztest debug release help master clean"
...
function fn_build_googletest()
{
    ...
    cd $CACHE_DIR
    wget --no-check-certificate https://mindie.obs.cn-north-4.myhuaweicloud.com/open_source/googletest-1.13.0.tar.gz
...
function fn_build_stub()
{
    ...
    wget --no-check-certificate https://mindie.obs.cn-north-4.myhuaweicloud.com/open_source/cpp-stub-master.tar.gz
...
function fn_build_mockcpp()
{
    if [ -d "$THIRD_PARTY_DIR/mockcpp/lib" -a -d "$THIRD_PARTY_DIR/mockcpp/include" ]; then
        return $?
    fi

    if [ -d "$THIRD_PARTY_DIR/mockcpp/" ]; then
        cd $THIRD_PARTY_DIR/mockcpp/src/2.7
    else
        cd $CACHE_DIR
        git clone https://gitee.com/sinojelly/mockcpp.git
        cd "${CACHE_DIR}/mockcpp"
        git checkout -f tags/v2.7
    fi
    patch -p1 -f < $CODE_ROOT/tests/update.patch || true
    ...
    cmake .. -DCMAKE_INSTALL_PREFIX=$THIRD_PARTY_DIR/mockcpp -DCMAKE_SKIP_RPATH=TRUE -DCMAKE_CXX_FLAGS="-fPIC"
    cmake --build . --parallel $(nproc)
    cmake --install .
    echo "mockcpp is successfully installed to $THIRD_PARTY_DIR/mockcpp"
}
...
function fn_build_nlohmann_json()
{
    ...
    if [ ! -d "$NLOHMANN_DIR" ];then
        if [ ! -f "$CACHE_DIR/nlohmann/include.zip" ];then
            ...
            mkdir nlohmann
            cd nlohmann
            wget --no-check-certificate https://mindie.obs.cn-north-4.myhuaweicloud.com/open_source/nlohmanJson/v3.11.2/include.zip
        else
...
function fn_build_spdlog()
{
    SPDLOG_DIR=$THIRD_PARTY_DIR/spdlog
    if [ ! -d "$SPDLOG_DIR/include" ]; then
        cd $THIRD_PARTY_DIR
        rm -rf spdlog
        wget https://mindie.obs.cn-north-4.myhuaweicloud.com/open_source/spdlog/v1.12.0.zip --no-check-certificate
        unzip v1.12.0.zip
        mv spdlog-1.12.0 spdlog
        rm -f v1.12.0.zip
        cd -
    fi
}
...
function fn_run_fuzztest()
{
    ...
    test_list=$($OUTPUT_DIR/atb_models/bin/speed_fuzztest --gtest_list_tests | grep -v '^#' | grep -v '^$')
    test_cases=()
    ...
    for test_case in "${test_cases[@]}"; do
        $OUTPUT_DIR/atb_models/bin/speed_fuzztest --gtest_filter="$test_case*"

        $LCOV_PATH -c -d $CACHE_DIR -o $GCOV_INFO_DIR/$test_case.info --rc lcov_branch_coverage=1 >> $GCOV_DIR/log.txt
        $LCOV_PATH -a $GCOV_INFO_DIR/$test_case.info -o $GCOV_INFO_DIR/cover.info --rc lcov_branch_coverage=1 >> $GCOV_DIR/log.txt
        rm $GCOV_INFO_DIR/$test_case.info
    done
}
...
function fn_files_authority_limit()
{
    find tests/modeltest -type f \( -name "*.py" -o -name "*.sh" -o -name "*.cpp" -o -name "*.cpp" -o -name "*.mod" -o -name "*.sum" -o -name "*.tar.gz" \) -exec chmod 550 {} \;
    find tests/modeltest -type f \( -name "*.json" -o -name "*.jsonl" -o -name "*.csv" \) -exec chmod 640 {} \;
    find tests/modeltest -type d -exec chmod 550 {} +
    chmod 750 tests/modeltest
}
...
function fn_extract_debug_symbols() {
    ...
    find $in_dir -type f \( -executable -o -name "*.so*" \) | while read -r binary; do
        # Only ELF file has symbols to strip
...
function fn_main()
{
    ...
    case "${arg1}" in
        ...
        "clean")
            ...
            ;;
        "help")
            echo "build.sh third_party|unittest|pythontest|fuzztest|debug|release|master --skip_build|--gcov|--no_hostbin|--no_devicebin|--output=<dir>|--cache=<dir>|--use_cxx11_abi=0|--use_cxx11_abi=1|--build_config=<path>"
            ;;
        *)
