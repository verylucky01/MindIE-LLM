...
namespace atb_speed {
    namespace common {
        ...
        void CheckParam(const AttentionParam &param)
        {
            if (param.numAttentionHeads == 0) {
                std::stringstream ss;
                ss << "Cannot be devided by zero. Param numAttentionHeads is zero!" << std::endl;
                throw std::runtime_error(ss.str());
            }
            if (param.numKeyValueHeads == 0) {
                std::stringstream ss;
                ss << "Cannot be devided by zero. Param numKeyValueHeads is zero!" << std::endl;
                throw std::runtime_error(ss.str());
            }
            if (param.hiddenSize == 0) {
                std::stringstream ss;
                ss << "Cannot be devided by zero. Param hiddenSize is zero!" << std::endl;
                throw std::runtime_error(ss.str());
            }
            if (param.useQKNorm && param.hiddenSizePerAttentionHead == 0) {
                std::stringstream ss;
