...
{
    sizeSplits = aclCreateIntArray(param_.sizeSpilts.data(), param_.sizeSpilts.size());
}
...
atb::Status DequantRopeQuantKvcacheOperation::CreateAclNNInTensorVariantPack(const atb::VariantPack &variantPack)
{
    AclNNVariantPack &aclnnVariantPack = this->aclnnOpCache_->aclnnVariantPack;
...
atb::Status DequantRopeQuantKvcacheOperation::CreateAclNNOutTensorVariantPack(const atb::VariantPack &variantPack)
{
    AclNNVariantPack &aclnnVariantPack = this->aclnnOpCache_->aclnnVariantPack;
    aclnnVariantPack.aclOutTensors.resize(GetOutputNum());
    for (size_t i = 0; i < aclnnVariantPack.aclOutTensors.size(); ++i) {
        CHECK_OPERATION_STATUS_RETURN(CreateTensor(variantPack.outTensors.at(i), i, aclnnVariantPack.aclOutTensors[i]));
    }
    return atb::NO_ERROR;
}
