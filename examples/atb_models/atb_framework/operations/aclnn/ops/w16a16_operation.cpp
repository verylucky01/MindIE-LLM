    ...
    AclNNMatmulParam param) : AclNNOperation(name), param_(param)
{
    ...
    float oneValue = 1.0f;
    betaZero = aclCreateScalar(&zeroValue, aclDataType::ACL_FLOAT);
    betaOne = aclCreateScalar(&oneValue, aclDataType::ACL_FLOAT);
}
...
atb::Status W16A16Operation::InferShape(
    const atb::SVector<atb::TensorDesc> &inTensorDescs, atb::SVector<atb::TensorDesc> &outTensorDescs) const
{
    LOG_DEBUG_MODEL << opName_ << "W16A16Operation infer shape start";
    ...
    LOG_DEBUG_MODEL << opName_ << "W16A16Operation infer shape origin inTensorDescs.at(DIM0).shape.dims[DIM0]"
                  << inTensorDescs.at(DIM0).shape.dims[DIM0];
    LOG_DEBUG_MODEL << opName_ << "W16A16Operation infer shape origin inTensorDescs.at(DIM1).shape.dims[nDim]"
    ...
    LOG_DEBUG_MODEL << opName_ << "W16A16Operation infer shape end";
    return 0;
}

uint32_t W16A16Operation::GetInputNum() const
{
...
uint32_t W16A16Operation::GetOutputNum() const
{
    return DIM1;
}

atb::Dims W16A16Operation::GetWeightStorageShape(const atb::TensorDesc atbTensorDesc) const
{
    ...
    if (atbTensorDesc.format == ACL_FORMAT_FRACTAL_NZ) {
        ...
        uint32_t blockSize = 16; // 16: 以blockSize大小16进行切分
...
atb::Status W16A16Operation::CreateAclNNInTensorVariantPack(const atb::VariantPack &variantPack)
{
...
atb::Status W16A16Operation::CreateAclNNOutTensorVariantPack(const atb::VariantPack &variantPack)
{
    AclNNVariantPack &aclnnVariantPack = this->aclnnOpCache_->aclnnVariantPack;
...
int W16A16Operation::SetAclNNWorkspaceExecutor()
{
    ...
    int ret = aclnnAddmmGetWorkspaceSize(
        ...
        param_.hasBias ? betaOne : betaZero,
        betaOne,
        aclnnVariantPack.aclOutTensors.at(DIM0)->tensor,
        0,
...
int W16A16Operation::ExecuteAclNNOp(uint8_t *workspace, aclrtStream &stream)
{
