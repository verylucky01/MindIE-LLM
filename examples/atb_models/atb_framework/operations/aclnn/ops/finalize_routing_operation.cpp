...
{
    LOG_DEBUG_MODEL << opName_ << "FinalizeRoutingOperation infer shape start";
    ...
    LOG_DEBUG_MODEL << opName_ << "FinalizeRoutingOperation infer shape origin inTensorDescs.at(DIM0).shape.dims[DIM0]"
    ...
    LOG_DEBUG_MODEL << opName_ << "FinalizeRoutingOperation infer shape end";
    return 0;
}
...
{
    LOG_DEBUG_MODEL << opName_ << " FinalizeRoutingOperation start";
...
int FinalizeRoutingOperation::ExecuteAclNNOp(uint8_t *workspace, aclrtStream &stream)
{
    ...
    LOG_DEBUG_MODEL << opName_ << " FinalizeRoutingOperation end, ret:" << ret;
    return ret;
}
