...
{
    LOG_DEBUG_MODEL << opName_ << "DynamicQuantOperation infer shape start";
    ...
    LOG_DEBUG_MODEL << opName_ << "DynamicQuantOperation infer shape origin inTensorDescs.at(DIM0).shape.dims[DIM0]"
    ...
    LOG_DEBUG_MODEL << opName_ << "DynamicQuantOperation infer shape end";
    return 0;
}
...
{
    LOG_DEBUG_MODEL << opName_ << " DynamicQuantOperation start";
...
int DynamicQuantOperation::ExecuteAclNNOp(uint8_t *workspace, aclrtStream &stream)
{
    ...
    LOG_DEBUG_MODEL << opName_ << " DynamicQuantOperation end, ret:" << ret;
    return ret;
}
