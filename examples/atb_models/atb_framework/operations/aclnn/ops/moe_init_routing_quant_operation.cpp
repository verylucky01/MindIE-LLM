...
MoeInitRoutingQuantOperation::MoeInitRoutingQuantOperation(
    const std::string &name, MoeInitRoutingQuantParam param) : AclNNOperation(name), param_(param) {}
MoeInitRoutingQuantOperation::~MoeInitRoutingQuantOperation() {}
    ...
    const atb::SVector<atb::TensorDesc> &inTensorDescs, atb::SVector<atb::TensorDesc> &outTensorDescs) const
{
    ...
    outTensorDescs.at(DIM3).dtype = aclDataType::ACL_INT32;
...
uint32_t MoeInitRoutingQuantOperation::GetOutputNum() const
{
    return NUM5;
}
...
int MoeInitRoutingQuantOperation::SetAclNNWorkspaceExecutor()
{
    ...
    int outputSize = GetMoeInitRoutingOpOutputSize(inputSize, param_);
    int ret = aclnnMoeInitRoutingQuantV2GetWorkspaceSize(
        aclnnVariantPack.aclInTensors.at(DIM0)->tensor,
        aclnnVariantPack.aclInTensors.at(DIM1)->tensor,
        nullptr,
        nullptr,
        outputSize,
        0, param_.expertNum, 0, param_.expertTokensCoutOrCumsumFlag, false, param_.quantMode,
        ...
        aclnnVariantPack.aclOutTensors.at(DIM3)->tensor,
