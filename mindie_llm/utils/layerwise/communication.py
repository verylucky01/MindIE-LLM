...
class LwdCommunicationManager:
    ...
    def communication_config_check(self, model_config: DmiConfig) -> bool:
        ...
        if npu_edge_num not in [2, 4, 8] or npu_cloud_num not in [8]: 
            logger.error(
                        "[layerwiseDisaggregated] The configuration is invalid because " \
        ...
        if not LwdCommunicationManager.is_number_in_range(int(tcp_comm_cloud_ip_port1), 1024, 65535) or \
            not LwdCommunicationManager.is_number_in_range(int(tcp_comm_cloud_ip_port2), 1024, 65535):
            logger.error(
                        "[layerwiseDisaggregated] The configuration is invalid because " \
        ...
        if not LwdCommunicationManager.is_number_in_range(int(self.hccl_comm_edge_ip_port), 1024, 65535):
            logger.error(
                        "[layerwiseDisaggregated] The configuration is invalid because " \
    ...
    def communication_init(self) -> bool:
        ...
        self.ctrl_comm.init_tcp_link(rank=self.rank, role=self.role_type,
                                     server_ip=self.edge_ip_address, server_port=self.tcp_comm_cloud_ip_port)
    ...
    def parse(self, model_config: DmiConfig):
        ...
        self.cloud_ip_address = model_config.parse_list("layerwiseDisaggregatedSlaveIpAddress", \
                                                      required=False, default_value=None)
