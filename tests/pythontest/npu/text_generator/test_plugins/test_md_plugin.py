...
class TestPlugin(unittest.TestCase):
    ...
    def test_generate_token_greedy(self):
        ...
        with patch.object(GeneratorTorch, 'forward') as mock_forward, \
            ...
            generation_output = md_plugin.generate_token(meta_data)
    ...
    def test_decoding_length_max(self):
        def side_effect_initialize_distributed(rank, npu_id, world_size):
            return FakeGroup(rank, world_size), torch.device("cpu")
        ...
        with patch.object(GeneratorTorch, 'forward') as mock_forward, \
            ...
            self.model_config[
                "plugin_params"] = '{\"plugin_type\": \"memory_decoding\",\"decoding_length\":17,\"dynamic_algo\":true}'
            generator = Generator(self.model_config)
            md_plugin = generator.plugin
            self.assertEqual(md_plugin.memory_decoding.decoding_length, 16)

    def test_decoding_length_min(self):
        def side_effect_initialize_distributed(rank, npu_id, world_size):
            return FakeGroup(rank, world_size), torch.device("cpu")
        ...
        with patch.object(GeneratorTorch, 'forward') as mock_forward, \
            ...
            mock_forward.side_effect = side_effect_forward
            self.model_config[
                "plugin_params"] = '{\"plugin_type\": \"memory_decoding\",\"decoding_length\":0,\"dynamic_algo\":true}'
            generator = Generator(self.model_config)
            md_plugin = generator.plugin
            self.assertEqual(md_plugin.memory_decoding.decoding_length, 1)
